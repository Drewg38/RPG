(function(){
  ...
  if (!startId) { console.error("[game.js] No pages in RPG_CONTENT."); }

  var currentId = startId;
  var current   = startId ? pages[startId] : null;

  // ---- State ----
  var state = {
    selectedChoice: -1,
    d: { x:0,y:0,r:52,vx:0,vy:0,ang:0,vang:0,grabbed:false,value:null,last:{x:0,y:0} },
  ...
  // ---- Dice ----
  function drawD20(cx,cy,rad,ang,val){
    var body=ctx.createRadialGradient(-rad*0.4,-rad*0.4,rad*0.3,0,0,rad);
    body.addColorStop(0,"#9de1ff"); body.addColorStop(0.6,"#5ea7ff"); body.addColorStop(1,"#6d42ff");
    ctx.save(); ctx.translate(cx,cy); ctx.rotate(ang);
    ctx.beginPath();
    for(var i=0;i<20;i++){ var a=(i/20)*Math.PI*2; var rr2=rad*(0.9+0.08*Math.sin(i*1.7)); var x=Math.cos(a)*rr2, y=Math.sin(a)*rr2; if(i===0) ctx.moveTo(x,y); else ctx.lineTo(x,y); }
    ctx.closePath();
    ctx.save(); ctx.shadowColor="rgba(0,0,0,0.40)"; ctx.shadowBlur=14; ctx.shadowOffsetX=0; ctx.shadowOffsetY=4; ctx.fillStyle=body; ctx.fill(); ctx.restore();
    ctx.globalAlpha=0.35; ctx.strokeStyle="#0b0c10"; ctx.lineWidth=1; ctx.stroke(); ctx.globalAlpha=1;
    if(val!=null){ ctx.fillStyle="#0b0c10"; var size=Math.floor(rad*0.7); ctx.font="bold "+size+"px system-ui, sans-serif"; ctx.textAlign="center"; ctx.textBaseline="middle"; ctx.fillText(String(val),0,2); }
    ctx.restore();
    ctx.fillStyle="rgba(0,0,0,0.35)"; ctx.beginPath(); ctx.ellipse(cx+6, cy+8, rad*0.9, rad*0.3, 0, 0, Math.PI*2); ctx.fill();
  }

  // ---- Layout ----
  function layout(){
    var W = canvas.width / DPR, pad=16, lineH=18;
    var imgH=280, textW=W-pad*2-28, h=26;
    ...
    state.rects.tray = {x:pad,y:trayTop,w:W-pad*2,h:180};
    ...
    if(need > cssH + 1){ canvas.style.height = need+"px"; canvas.height = Math.floor(need*DPR); ctx.setTransform(DPR,0,0,DPR,0,0); }
  ...
  function draw(){
    ...
    if(r){
      ...
      var g=ctx.createLinearGradient(x,y-18,x+320,y+6); g.addColorStop(0,"#93c5fd"); g.addColorStop(1,"#a78bfa"); ctx.fillStyle=g;
      ctx.font="800 22px system-ui, sans-serif"; ctx.fillText((current && current.title) || "Untitled", x, y); y+=26;
    ...
    if(t){
      rr(t.x,t.y,t.w,t.h,14);
      var edge=ctx.createLinearGradient(t.x,t.y,t.x,t.y+t.h); edge.addColorStop(0,"rgba(43,49,64,0.30)"); edge.addColorStop(1,"rgba(26,31,42,0.20)");
      ctx.fillStyle=edge; ctx.fill();
      rr(t.x+8,t.y+8,t.w-16,t.h-16,10);
      var deck=ctx.createLinearGradient(t.x,t.y,t.x,t.y+t.h); deck.addColorStop(0,"rgba(12,17,26,0.18)"); deck.addColorStop(1,"rgba(10,13,18,0.14)");
      ...
      drawD20(state.d.x,state.d.y,state.d.r,state.d.ang,state.d.value);
    }

    var cards=state.rects.choices; var choices=(current && current.choices) || [];
    for(var i=0;i<cards.length;i++){
      ...
      var text=(choices[i] && (choices[i].label || choices[i].text || choices[i].title)) || "-";
      wrap(text, c.x+14, c.y+44, c.w-28, 18, "600 14px system-ui, sans-serif");
    }

    var h=state.rects.hint;
    if(h){
      var grad=ctx.createLinearGradient(h.x,h.y,h.x+h.w,h.y+24); grad.addColorStop(0,"#93c5fd"); grad.addColorStop(1,"#c4b5fd");
      ctx.save(); ctx.fillStyle=grad; ctx.shadowColor="rgba(0,0,0,0.35)"; ctx.shadowBlur=8; ctx.font="800 20px system-ui, sans-serif";
      ctx.fillText("After you choose, drag the die to roll.", h.x, h.y+18); ctx.restore();
    }

    var rres=state.rects.result;
    if(rres && state.d && state.d.value!=null){
      var g2=ctx.createLinearGradient(rres.x,rres.y,rres.x+320,rres.y+24); g2.addColorStop(0,"#93c5fd"); g2.addColorStop(1,"#a78bfa");
      ctx.save(); ctx.fillStyle=g2; ctx.font="800 22px system-ui, sans-serif"; var msg="Roll result: "+state.d.value; ctx.fillText(msg, rres.x, rres.y+18); ctx.restore();
    }
  ...
  window.addEventListener("mousemove", function(e){
    ...
    var my=(e.clientY-rect.top)*(canvas.height/rect.height)/DPR;
    var t=state.rects.tray;
    state.d.x = clamp(mx, t.x+10+state.d.r, t.x+t.w-10-state.d.r);
    state.d.y = clamp(my, t.y+10+state.d.r, t.y+t.h-10-state.d.r);
    var dx=mx-state.d.last.x, dy=my-state.d.last.y;
    state.d.vx=dx*1.2; state.d.vy=dy*1.2; state.d.vang=(dx-dy)*0.04;
