/* game.js (v2) — Canvas renderer + branching engine using RPG_CONTENT (CSV-only).
   - No built-in scenarios; requires content.js to set window.RPG_CONTENT { pages, startId }.
   - Supports page images, multiple choices leading to next pages, and simple fight scenes.
*/
...
function roundRectPath(x,y,w,h,r){ r=Math.max(0,r||10); ctx.beginPath(); ctx.moveTo(x+r,y); ctx.arcTo(x+w,y,x+w,y+h,r); ctx.arcTo(x+w,y+h,x,y+h,r); ctx.arcTo(x,y+h,x,y,r); ctx.arcTo(x,y,x+w,y,r); ctx.closePath(); }
...
const engine = {
  ...
  // fight state
  fight:null // { enemy, hp, ac, dmg, playerHP }
};
...
function layout(){
  ...
  // Left controls (activities/stats) + Right tray
...
window.addEventListener('mouseup', ()=>{
  const d=engine.d20; if(!d.grabbed) return; d.grabbed=false; d.vx*=2.6; d.vy*=2.6; d.vang*=2.0;
...
function resolveFight(playerRoll){
  ...
  if (f.enemyHP <= 0){
    ...
    engine.fight = null; // clear fight; continue with choices on next roll/selection
...
function boot(){
  // Wait a tick for RPG_CONTENT to be set (content.js)
